...
function receiptCatLabel(tr, c) {
  const key = `receipt_cat_${String(c).replace(/[^A-Za-z]/g, '')}`
  return tr[key] || c
}
...
export default function Receipts({ session }) {
  ...
  const exportPDF = useCallback(async () => {
    ...
    doc.text(tr.receipt_pdf_title, 20, 20)
    doc.setFontSize(10); doc.setTextColor(100)
    doc.text(`${tr.receipt_pdf_generated}: ${new Date().toLocaleDateString()}`, 20, 30)
    let y = 45
    doc.setFillColor(240,240,240); doc.rect(15, y-5, 180, 8, 'F')
    doc.setTextColor(0)
    doc.text(tr.receipt_pdf_merchant, 20, y); doc.text(tr.receipt_pdf_category, 80, y)
    doc.text(tr.receipt_pdf_date, 120, y); doc.text(tr.receipt_pdf_amount, 165, y)
    y += 10
    receipts.forEach((r,i) => {
      ...
      doc.text(String(receiptCatLabel(tr, r.category)||''), 80, y)
    ...
    doc.text(`${tr.receipt_pdf_total}: $${totalSpent.toFixed(2)}`, 140, y)
    doc.save(`Receipts-${new Date().toISOString().split('T')[0]}.pdf`)
  }, [receipts, totalSpent, tr])
  ...
  return (
    <div style={{ paddingBottom:100 }}>
      <div style={{ background:'linear-gradient(135deg, #534AB7, #342D8A)', borderRadius:'16px 16px 0 0', padding:'18px 16px 28px', marginBottom:'-14px', color:'white' }}>
        ...
        <h2 style={{ color:'white', margin:'0 0 4px', fontSize:22, fontWeight:800 }}>{tr.receipt_title}</h2>
        <p style={{ color:'rgba(255,255,255,0.8)', margin:0, fontSize:13 }}>{tr.receipt_subtitle}</p>
      </div>
      ...
      <div style={{ paddingTop:24 }}>
        {receipts.length > 0 && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:16 }}>
            <div className="card" style={{ textAlign:'center' }}>
              <div style={{ fontSize:11, color:'var(--text-muted)', marginBottom:4 }}>{tr.receipt_total_count}</div>
              <div style={{ fontSize:24, fontWeight:800, color:'var(--green)' }}>{receipts.length}</div>
            </div>
            <div className="card" style={{ textAlign:'center' }}>
              <div style={{ fontSize:11, color:'var(--text-muted)', marginBottom:4 }}>{tr.receipt_total_spent}</div>
        ...
        <div className="card" style={{ marginBottom:16 }}>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:12 }}>{tr.receipt_add_section}</div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
            <button type="button" onClick={() => cameraRef.current.click()}
              ...
              <div style={{ fontSize:11, fontWeight:700 }}>{tr.receipt_camera}</div>
            </button>
            ...
            <button type="button" onClick={() => fileRef.current.click()}
              ...
              <div style={{ fontSize:11, fontWeight:700 }}>{tr.receipt_gallery}</div>
            </button>
            ...
            <button type="button" onClick={() => setShowModal(true)}
              ...
              <div style={{ fontSize:24, marginBottom:4 }}>✏️</div>
              <div style={{ fontSize:11, fontWeight:700 }}>{tr.receipt_manual}</div>
            </button>
          </div>
        </div>
        ...
        {byCat.length > 0 && (
          <div className="card" style={{ marginBottom:16 }}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:12 }}>{tr.receipt_by_cat}</div>
            {byCat.map((c,i) => (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}>
                    <span style={{ fontSize:13, fontWeight:600 }}>{receiptCatLabel(tr, c.cat)}</span>
        ...
        {receipts.length === 0 && (
          <div className="empty-state">
            ...
            <p>{loading ? tr.receipt_list_loading : tr.receipt_empty}</p>
            {!loading && <p style={{ fontSize:13, marginTop:8 }}>{tr.receipt_empty_hint}</p>}
          </div>
        )}
        ...
        {receipts.map((r,i) => (
          <div key={i} className="card" style={{ marginBottom:10, cursor:'pointer' }} onClick={() => setPreview(r)}>
            <div style={{ display:'flex', gap:12, alignItems:'center' }}>
              {r.image_url ? (
                <img src={r.image_url} alt="" style={{ width:56, height:56, borderRadius:10, objectFit:'cover', flexShrink:0 }}/>
              ) : (
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:12, color:'var(--text-muted)', marginTop:2 }}>{receiptCatLabel(tr, r.category)} · {r.date}</div>
                {r.tax > 0 && <div style={{ fontSize:11, color:'#BA7517', marginTop:1 }}>{tr.receipt_tax}: ${Number(r.tax).toFixed(2)}</div>}
              ...
              <div style={{ textAlign:'right', flexShrink:0 }}>
                ...
                <div style={{ fontSize:10, color:'var(--text-muted)', marginTop:2 }}>{tr.receipt_tap_view}</div>
              </div>
            </div>
          </div>
        ))}
        ...
        {receipts.length > 0 && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:16 }}>
            <div className="card" style={{ background:'linear-gradient(135deg, #A32D2D, #6B1A1A)', color:'white' }}>
              <div style={{ fontSize:11, opacity:0.8 }}>{tr.receipt_grand_total}</div>
              <div style={{ fontSize:22, fontWeight:900 }}>${totalSpent.toFixed(2)}</div>
              <div style={{ fontSize:11, opacity:0.7 }}>{interpolate(tr.receipt_count_suffix, { n: receipts.length })}</div>
            </div>
            <button type="button" onClick={exportPDF}
              ...
              <div style={{ fontSize:24, marginBottom:4 }}>📄</div>
              <div style={{ fontSize:13, fontWeight:700 }}>{tr.receipt_export_pdf}</div>
            </button>
          </div>
        )}
      </div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxHeight:'90vh', overflowY:'auto' }}>
            {scanning ? (
              <div style={{ textAlign:'center', padding:'30px 16px' }}>
                ...
                <div style={{ fontWeight:700, fontSize:17, marginBottom:8 }}>{tr.receipt_scanning_title}</div>
                <div style={{ fontSize:13, color:'var(--text-muted)', marginBottom:20 }}>{tr.receipt_scanning_sub}</div>
                <div className="spinner" style={{ margin:'0 auto' }}/>
              </div>
            ) : (
              <>
                <div style={{ fontWeight:700, fontSize:16, marginBottom:12 }}>
                  {filePreview ? tr.receipt_modal_scanned : tr.receipt_modal_add}
                </div>
                {filePreview && (
                  <img src={filePreview} alt="" style={{ width:'100%', height:140, objectFit:'cover', borderRadius:10, marginBottom:12 }}/>
                )}
                <div className="form-group" style={{ marginBottom:10 }}>
                  <label>{tr.receipt_label_amount}</label>
                  <input type="number" placeholder={tr.receipt_ph_amount} value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))} min="0" step="0.01"/>
                </div>
                <div className="form-group" style={{ marginBottom:10 }}>
                  <label>{tr.receipt_label_tax}</label>
                  <input type="number" placeholder="0.00" value={form.tax} onChange={e=>setForm(f=>({...f,tax:e.target.value}))} min="0" step="0.01"/>
                </div>
                <div className="form-group" style={{ marginBottom:10 }}>
                  <label>{tr.receipt_label_merchant}</label>
                  <input type="text" placeholder={tr.receipt_ph_merchant} value={form.merchant} onChange={e=>setForm(f=>({...f,merchant:e.target.value}))}/>
                </div>
                <div className="form-group" style={{ marginBottom:10 }}>
                  <label>{tr.receipt_label_category}</label>
                  <select value={form.category} onChange={e=>setForm(f=>({...f,category:e.target.value}))}>
                    {CATEGORIES.map(c => <option key={c} value={c}>{receiptCatLabel(tr, c)}</option>)}
                  </select>
                </div>
                <div className="form-group" style={{ marginBottom:10 }}>
                  <label>{tr.receipt_label_date}</label>
                  <input type="date" value={form.date} onChange={e=>setForm(f=>({...f,date:e.target.value}))}/>
                </div>
                <div className="form-group" style={{ marginBottom:16 }}>
                  <label>{tr.receipt_label_notes}</label>
                  <input type="text" placeholder={tr.receipt_ph_notes} value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))}/>
                </div>
                <div style={{ display:'flex', gap:10 }}>
                  <button type="button" onClick={() => { setShowModal(false); setSelectedFile(null); setFilePreview(null) }}
                    style={{ flex:1, padding:'13px', background:'#f3f4f6', border:'none', borderRadius:10, fontWeight:600, cursor:'pointer' }}>
                    {tr.cancel}
                  </button>
                  <button type="button" onClick={saveReceipt} disabled={uploading||!form.amount||!form.merchant}
                    ...
                    {uploading ? tr.receipt_saving : tr.receipt_save}
                  </button>
                </div>
              </>
            )}
      ...
      {preview && (
        <div className="modal-overlay" onClick={() => setPreview(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div style={{ fontWeight:700, fontSize:16, marginBottom:12 }}>{tr.receipt_details}</div>
            {preview.image_url && (
              <img src={preview.image_url} alt="" style={{ width:'100%', maxHeight:200, objectFit:'contain', borderRadius:10, marginBottom:12, background:'#f3f4f6' }}/>
            )}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:12 }}>
              {[
                { label: tr.receipt_pdf_amount, value:`$${Number(preview.amount).toFixed(2)}`, color:'#A32D2D' },
                { label: tr.receipt_label_tax, value:`$${Number(preview.tax||0).toFixed(2)}`, color:'#BA7517' },
                { label: tr.receipt_pdf_merchant, value:preview.merchant },
                { label: tr.receipt_pdf_category, value: receiptCatLabel(tr, preview.category) },
                { label: tr.receipt_pdf_date, value:preview.date },
                { label: tr.receipt_label_total_tax, value:`$${(Number(preview.amount)+Number(preview.tax||0)).toFixed(2)}`, color:'#A32D2D' },
              ].map((item,i) => (
            ...
            {preview.notes && (
              <div style={{ padding:'10px 12px', background:'var(--bg)', borderRadius:8, border:'1px solid var(--border)', marginBottom:12 }}>
                <div style={{ fontSize:10, color:'var(--text-muted)', marginBottom:2 }}>{tr.receipt_label_notes}</div>
                <div style={{ fontSize:13 }}>{preview.notes}</div>
              </div>
            )}
            <div style={{ display:'flex', gap:10 }}>
              <button type="button" onClick={() => setPreview(null)}
                style={{ flex:1, padding:'12px', background:'#f3f4f6', border:'none', borderRadius:8, fontWeight:600, cursor:'pointer' }}>{tr.receipt_close}</button>
              <button type="button" onClick={() => deleteReceipt(preview.id)}
                style={{ flex:1, padding:'12px', background:'#A32D2D', color:'white', border:'none', borderRadius:8, fontWeight:600, cursor:'pointer' }}>{tr.receipt_delete}</button>
            </div>
          </div>
        </div>
      )}
